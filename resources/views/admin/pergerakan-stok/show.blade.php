    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 w-full overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ (str_contains(url()->previous(), route('admin.pergerakan-stok.index')) ? url()->previous() : route('admin.pergerakan-stok.index', ['tab' => $pergerakanStok->jenis_pergerakan])) }}" 
            ...
            <button type="button" data-swal-delete data-url="{{ route('admin.pergerakan-stok.destroy', $pergerakanStok) }}" data-method="DELETE" data-message="Transaksi {{ $pergerakanStok->jenis_pergerakan === 'masuk' ? 'stok masuk' : 'stok keluar' }} '{{ $pergerakanStok->nomor_transaksi }}' akan dihapus. Stok bahan baku akan dikembalikan." class="inline-flex items-center gap-2 px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 text-sm font-medium rounded-xl transition-colors cursor-pointer">
