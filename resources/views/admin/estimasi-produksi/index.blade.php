<x-layouts.admin>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div class="px-6 py-3 bg-gray-50/50 border-b border-gray-100 flex flex-col sm:flex-row items-center gap-4 relative z-20">
            ...
            @if(request()->hasAny(['search', 'kategori', 'status', 'sort']))
                <a href="{{ route('admin.estimasi-produksi.index') }}" title="Hapus Semua Filter & Pencarian" class="hidden sm:flex items-center justify-center w-10 h-10 bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-600 rounded-xl transition-colors shrink-0">
            ...
            <!-- KANAN: Search Bar -->
            <form method="GET" action="{{ route('admin.estimasi-produksi.index') }}" class="relative flex-1">
                                ...
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="action-dropdown w-40 bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.12)] border border-gray-100 opacity-0 scale-95 pointer-events-none transition-all duration-200 z-50 origin-top-right hidden py-1">
                                            ...
                                            <div class="border-t border-gray-100 my-1"></div>
                                            <form action="{{ route('admin.estimasi-produksi.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus baseline ini? Data terkait mungkin akan terpengaruh.');" class="w-full">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-8">
            <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl transform scale-95 transition-transform duration-300 flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 overflow-y-auto">
                    <form action="{{ route('admin.estimasi-produksi.store') }}" method="POST" id="addForm">
                        @csrf
                        <div class="space-y-4">
                            <!-- Pilih Produk -->
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-1">Produk <span class="text-red-500">*</span></label>
                                <select name="produk_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-1 focus:ring-[#0F034D]/20 focus:border-[#0F034D] transition-colors text-sm bg-white cursor-pointer">
                                    <option value="">— Pilih Produk —</option>
                                    @foreach($produks as $produk)
                                        <option value="{{ $produk->id }}">{{ $produk->nama_produk }} ({{ $produk->kode_produk }}) — {{ ucfirst($produk->ukuran) }}, {{ $produk->warna }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="bahan_baku_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-1 focus:ring-[#0F034D]/20 focus:border-[#0F034D] transition-colors text-sm bg-white cursor-pointer">
                                    <option value="">— Pilih Bahan Baku —</option>
                                    @foreach($bahanBaku as $bahan)
                                        <option value="{{ $bahan->id }}">{{ $bahan->nama_bahan }} ({{ $bahan->kode_bahan }}) — {{ $bahan->warna }}, {{ ucfirst($bahan->kategori) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                                    ...
                                    <input type="checkbox" name="is_aktif" value="1" checked class="w-4 h-4 text-[#0F034D] rounded focus:ring-[#0F034D]">
                                    <div>
                ...
                <div class="p-6 overflow-y-auto">
                    <form action="" method="POST" id="editForm">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-1">Produk <span class="text-red-500">*</span></label>
                                <select name="produk_id" id="edit_produk_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-1 focus:ring-[#0F034D]/20 focus:border-[#0F034D] transition-colors text-sm bg-white cursor-pointer">
                                    <option value="">— Pilih Produk —</option>
                                    @foreach($produks as $produk)
                                        <option value="{{ $produk->id }}">{{ $produk->nama_produk }} ({{ $produk->kode_produk }}) — {{ ucfirst($produk->ukuran) }}, {{ $produk->warna }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-1">Bahan Baku <span class="text-red-500">*</span></label>
                                <select name="bahan_baku_id" id="edit_bahan_baku_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-1 focus:ring-[#0F034D]/20 focus:border-[#0F034D] transition-colors text-sm bg-white cursor-pointer">
                                    <option value="">— Pilih Bahan Baku —</option>
                                    @foreach($bahanBaku as $bahan)
                                        <option value="{{ $bahan->id }}">{{ $bahan->nama_bahan }} ({{ $bahan->kode_bahan }}) — {{ $bahan->warna }}, {{ ucfirst($bahan->kategori) }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                                    <input type="hidden" name="is_aktif" value="0">
                                    <input type="checkbox" name="is_aktif" id="edit_is_aktif" value="1" class="w-4 h-4 text-[#0F034D] rounded focus:ring-[#0F034D]">
                                    <div>
                                        <span class="text-sm font-semibold text-gray-700">Aktif</span>
                                        <p class="text-xs text-gray-500">Baseline ini akan digunakan dalam perhitungan produksi.</p>
                                    </div>
    ...
    @vite(['resources/js/admin/estimasi-produksi/toggle-modal.js'])
</x-layouts.admin>
