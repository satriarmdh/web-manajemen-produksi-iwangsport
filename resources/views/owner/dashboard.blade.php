<x-layouts.owner>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-2 space-y-6">
            <!-- Card 1: Tren Penjualan (Bar Chart Berfilter) -->
            ...
            <!-- Card 2: Perintah Produksi Terbaru -->
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 pb-4 border-b border-gray-50">
                    <div>
                        <h3 class="text-base font-bold text-gray-900">Perintah Produksi Terbaru</h3>
                        <p class="text-xs text-gray-500 mt-0.5">Pantauan siklus dan status perintah produksi (Work Order) berjalan.</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <a href="{{ route('owner.perintah-produksi.index') }}" class="text-xs font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-100 px-3 py-1.5 rounded-lg transition-colors cursor-pointer shrink-0">
                            Persetujuan WO
                        </a>
                        <a href="{{ route('owner.pantau-progres.index') }}" class="text-xs font-bold text-[#0F034D] bg-[#0F034D]/5 hover:bg-[#0F034D]/10 px-3 py-1.5 rounded-lg transition-colors cursor-pointer shrink-0">
                            Pantau Progres
                        </a>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto -mx-6 -mb-6">
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50/80 border-b border-gray-100">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nomor WO</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Dibuat Oleh</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Estimasi Produk</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Waktu Pembuatan</th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50 bg-white">
                            @forelse($recentActivity['perintahProduksi'] as $wo)
                                @php
                                    $statusText = match ($wo->status_produksi) {
                                        'pending' => 'Pending',
                                        'disetujui' => 'Disetujui',
                                        'dalam_produksi' => 'Diproses',
                                        'selesai' => 'Selesai',
                                        'ditolak' => 'Ditolak',
                                        default => ucfirst($wo->status_produksi),
                                    };
                                    $statusWarna = match ($wo->status_produksi) {
                                        'pending' => 'bg-amber-50 text-amber-700 border-amber-100',
                                        'disetujui' => 'bg-indigo-50 text-indigo-700 border-indigo-100',
                                        'dalam_produksi' => 'bg-blue-50 text-blue-700 border-blue-100',
                                    ...
                                    $produkSummary = $wo->details->map(fn($d) => ($d->produk->nama_produk ?? 'Produk') . ' (' . $d->estimasi_pcs . ' pcs)')->implode(', ');
                                @endphp
                                <tr class="hover:bg-gray-50/50 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-[#0F034D]">
                                        {{ $wo->nomor_wo }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-semibold text-gray-700 max-w-xs truncate" title="{{ $produkSummary }}">
                                            {{ $produkSummary ?: 'Tidak ada detail produk' }}
                                        </div>
                                        <div class="text-[10px] text-gray-400 mt-0.5">Total: {{ number_format($totalEstimasi, 0, ',', '.') }} pcs</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-400 tabular-nums">
                                        {{ $wo->created_at->diffForHumans() }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span class="inline-flex px-2.5 py-1 rounded-full text-[10px] font-bold border {{ $statusWarna }}">{{ $statusText }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        ...
                                        <p class="text-xs font-semibold text-gray-500">Belum ada perintah produksi</p>
                                        <p class="text-[11px] text-gray-400 mt-0.5">Perintah produksi baru dari admin akan muncul di sini.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <script>
        window.dashboardConfig = {
            initialSalesTrend: @json($salesTrend),
            salesTrendEndpoint: "{{ route('owner.dashboard.sales-trend') }}"
        };
    </script>
    @vite([
