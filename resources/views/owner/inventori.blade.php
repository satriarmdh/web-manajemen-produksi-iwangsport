<x-layouts.owner>
        ...
        <div id="content-mutasi-gudang" class="tab-content hidden relative z-30">
            ...
            <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Jejak perubahan stok yang direkam otomatis oleh sistem &mdash; mencakup inisiasi data, penambahan, pengurangan, dan penyesuaian bahan baku maupun produk secara kronologis.</p>
                </div>
    ...
    <script>
        ...
        let currentBahanStatus = "{{ request('stok') === 'menipis' ? 'menipis' : 'semua' }}";

        // State filter client-side untuk Produk Jadi
        let currentProdukSize = 'semua';
        let currentProdukStatus = "{{ request('stok') === 'menipis' ? 'menipis' : 'semua' }}";
        ...
        // Trigger filter awal jika URL memuat parameter stok menipis
        window.addEventListener('DOMContentLoaded', () => {
            // Bind listener ke input search untuk update reset button dinamis
            ...
            // Cek tab aktif dari parameter URL
            const urlParams = new URLSearchParams(window.location.search);
            const activeTab = urlParams.get('tab');
            if (activeTab) {
                switchTab(activeTab);
            } else if ("{{ request('stok') }}" === 'menipis') {
                filterBahanFromCard('menipis');
            }

            // Jaring pengaman: pastikan semua link pagination di tab mutasi
            // membawa parameter tab agar tidak kembali ke tab default saat pindah halaman.
            ...
            if (mutasiContent) {
                mutasiContent.querySelectorAll('a[href]').forEach(link => {
                    ...
                    } catch (e) { /* abaikan href non-standar */ }
                });
            }
        });
    </script>
