<x-layouts.owner>
    <x-slot:breadcrumb>
        ...
        <li class="flex items-center">
            ...
            <a href="{{ route('owner.mutasi-bahan-baku.index', ['tab' => $pergerakanStok->jenis_pergerakan]) }}" class="text-gray-400 hover:text-[#0F034D] transition-colors">Mutasi Bahan Baku</a>
        </li>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 w-full overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ (str_contains(url()->previous(), route('owner.mutasi-bahan-baku.index')) ? url()->previous() : route('owner.mutasi-bahan-baku.index', ['tab' => $pergerakanStok->jenis_pergerakan])) }}" 
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $pergerakanStok->jenis_pergerakan === 'masuk' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700' }}">
                            Bahan {{ $pergerakanStok->jenis_pergerakan === 'masuk' ? 'Masuk' : 'Keluar' }}
                        </span>
                    </div>
