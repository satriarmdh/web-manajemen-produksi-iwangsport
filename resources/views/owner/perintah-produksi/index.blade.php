<x-layouts.owner>
    ...
    <div class="space-y-5 mb-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                    <h3 class="text-lg font-bold text-[#0F034D]">Daftar WO Menunggu Persetujuan</h3>
                    <p class="text-sm text-gray-500 mt-1 max-w-3xl">Review work order dari admin sebelum masuk siklus produksi. Pastikan produk, penggunaan roll, dan estimasi hasil sudah sesuai.</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex flex-col sm:flex-row items-center gap-4">
            ...
            <form method="GET" action="{{ route('owner.perintah-produksi.index') }}" class="relative flex-1 w-full">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nomor WO atau nama admin pembuat..." class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-1 focus:ring-[#0F034D]/20 focus:border-[#0F034D] outline-none transition-colors bg-white shadow-sm">
            </form>
    ...
    @if($perintahProduksi->count() > 0)
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
            @foreach($perintahProduksi as $wo)
                ...
                @endphp
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-[0_12px_40px_rgba(15,3,77,0.08)] transition-all">
                    ...
                    <div class="rounded-xl bg-blue-50 border border-blue-100 p-3 mb-4">
                        <p class="text-xs text-blue-700 leading-relaxed">
                            Jika disetujui, WO akan berubah menjadi <span class="font-semibold">Disetujui</span> dan dapat dilanjutkan ke siklus produksi oleh tim terkait.
                        </p>
                    </div>

                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-3 border-t border-gray-50">
                        ...
                        <form action="{{ route('owner.perintah-produksi.reject', $wo) }}" method="POST" class="flex-1" onsubmit="return confirm('Tolak perintah produksi {{ $wo->nomor_wo }}?')">
                            @csrf
                            <input type="hidden" name="alasan_penolakan" value="Ditolak oleh owner setelah review work order">
    ...
    @else
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p class="text-[#0F034D] font-semibold text-sm">Tidak ada WO yang menunggu persetujuan</p>
            <p class="text-gray-500 text-sm mt-1">Semua work order sudah direview. WO baru dari admin akan muncul di halaman ini.</p>
        </div>
    @endif
</x-layouts.owner>
